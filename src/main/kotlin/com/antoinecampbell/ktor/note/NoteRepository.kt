...
class DefaultNoteRepository(
    ...
    override suspend fun save(note: Note): Note = dataSource.connection.use { connection ->
        connection
            .prepareStatement("INSERT INTO note(name) VALUES (?) RETURNING *")
            .apply {
    ...
    override suspend fun findById(id: Int): Note? = dataSource.connection.use { connection ->
        connection
            .prepareStatement("SELECT * FROM note WHERE id = ?")
            .apply {
                setInt(1, id)
            }.executeQuery()
    ...
    override suspend fun findAll(): List<Note> = dataSource.connection.use { connection ->
        connection
...
fun ResultSet.toNote(): Note = Note(
    id = getInt("id"),
    name = getString("name"),
